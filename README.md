...
```
┌────────────────────────────────────────────────────────┐
│ 📊 success-formula-analyzer (Skill)                    │
├────────────────────────────────────────────────────────┤
│ Input:  story_file_path (단일 스토리 파일)             │
│ Process: 패턴 추출 → 구조화된 데이터 리턴              │
│ Output:  • 10개 카테고리별 패턴                        │
│         • 구조화된 데이터 (JSON/마크다운)              │
│         • 파일 생성 없음 (데이터만 리턴)               │
│ Note:    pattern-classifier agent가 호출하여 사용      │
└────────────────────────────────────────────────────────┘
```
...
## 🔄 패턴 분류 시스템 (Pattern Classification System)
...
**2. 특정 스토리만 분류:**
```
Task agent: pattern-classifier
Input: research/stories/harvey-legal-ai-2025-12-16.md
```

**3. 모든 스토리 재분류 (주의: 시간 소요):**
```
Task agent: pattern-classifier
Input: "all"
```
...
**새 방식 (순차 처리):**
```
스토리 1: 5K (읽기) + 3K (분석) + 2K (업데이트) = 10K
스토리 2: 10K
스토리 3: 10K
...
9개 스토리: ~90K 토큰 (순차적, context 초기화)
```

**핵심 장점:**
- ✅ Context overflow 없음 (100+ 스토리 처리 가능)
- ✅ 각 스토리마다 fresh context
- ✅ 점진적 패턴 축적
- ✅ 스킬 재사용 가능
...
## Prerequisites
- Skills Required: [필요 스킬]
- Time Required: [시간]
- Capital Required: [자본]

## When to Use This Pattern
✅ Good Fit When: [적합한 경우]
❌ Bad Fit When: [부적합한 경우]
